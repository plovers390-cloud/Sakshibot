...
var Connection = class extends EventEmitter2 {
  ...
  /**
      ...
      */
  async connect() {
    if (this.state === 0 /* CONNECTING */ || this.state === 1 /* CONNECTED */) return;
...
var Player = class extends TypedEventEmitter {
  ...
  async move(name) {
    ...
    if (!node && ![...this.node.manager.nodes.values()].some((node2) => node2.state === 1 /* CONNECTED */))
      throw new Error("No available nodes to move to");
    if (!node || node.name === this.node.name || node.state !== 1 /* CONNECTED */) return false;
    let lastNode = this.node.manager.nodes.get(this.node.name);
    if (!lastNode || lastNode.state !== 1 /* CONNECTED */)
...
var Node = class extends TypedEventEmitter {
  ...
  disconnect(code, reason) {
    if (this.state !== 1 /* CONNECTED */ && this.state !== 0 /* CONNECTING */) return;
    this.state = 2 /* DISCONNECTING */;
    if (this.ws)
      this.ws.close(code, reason);
    else
      void this.close(1e3, Buffer.from(reason ?? "Unknown Reason", "utf-8"));
  }
  /**
